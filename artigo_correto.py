...
def apply_atrous_to_sequential_blocks(model: nn.Sequential, dilations: List[int] = [1,2,3]):
    """Apply atrous to dense layers within the sequential structure."""
    for i, module in enumerate(model.features):
        if 'DenseBlock' in module.__class__.__name__:
            layers = [m for m in module.children() if '_DenseLayer' in m.__class__.__name__]
            # Apply to last 3 layers
            for idx, d in enumerate(reversed(dilations), start=1):
                layer = layers[-idx]
                conv2 = layer.conv2
                new_conv2 = nn.Conv2d(
...
class A2SDNet121(nn.Module):
    def __init__(self, num_classes: int, pretrained: bool = True, reduction: int = 16):
        ...
        # ✅ 1. Replace Stem (Paper: Section "Stem layer")
        backbone = replace_stem(backbone)
        
        # ✅ 2. Insert SE blocks PRESERVING structure (Paper: "SE module" section)
        #    Don't rebuild into Sequential yet - just add SE modules
        self.backbone = self._insert_se_preserve_structure(backbone, reduction)
        
        # ✅ 3. Apply atrous to dense layers (Paper: "Atrous dense block" section)
        #    Now the denseblock{1..4} attributes still exist!
        self.backbone = apply_atrous_to_denseblocks(self.backbone, dilations=[1,2,3])
        
        # Build head
        # Get features from the final norm5 (after last denseblock)
        final_channels = self.backbone.features.norm5.num_features
        
        self.head = nn.Sequential(
            nn.Dropout(p=0.4),  # Paper mentions dropout in head
    ...
    def _insert_se_preserve_structure(self, model: models.DenseNet, reduction: int):
        """Insert SE blocks while KEEPING the original nested structure."""
        features = model.features
        
        # After each denseblock, add a SE module as an attribute
        # So we can call it in forward but preserve structure for atrous
        num_features_list = [
            features.transition1.conv.in_channels,  # After denseblock1
            features.transition2.conv.in_channels,  # After denseblock2
            features.transition3.conv.in_channels,  # After denseblock3
            features.norm5.num_features             # After denseblock4
        ]
        
        for i in range(1, 5):
            se = SEBlock(num_features_list[i-1], reduction=reduction)
            setattr(features, f'se_block_{i}', se)  # Store as attribute, not in Sequential
        
        return model

    def forward(self, x):
        # Manual forward to apply SE blocks at correct points
...
class A2SDLightning(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # StepLR as in paper (adjust every 30 epochs)
